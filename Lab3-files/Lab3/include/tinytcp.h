...
#define LINK_DELAY_BUF_SIZE 10000 //packets
...
typedef struct __attribute__((__packed__)) tinytcp_hdr {
    uint16_t src_port; //for client it's own
    uint16_t dst_port; //the server
    uint32_t seq_num;
    uint32_t ack_num;
...
extern int num_of_closed_conn_client;

extern int num_of_closed_conn_server;
