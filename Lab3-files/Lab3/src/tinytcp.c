...
int num_of_closed_conn_client = 0;
int num_of_closed_conn_server = 0;
                     ...
                     uint16_t tinytcp_pkt_size)
{
    ...
    for (int k = 0; k < 2; ++k) {
            ...
            if (x < loss_prob) { //drop or reorder
                if (delay_buff != NULL && delay_buff->pkt == NULL) {
                    ...
                    if (y < 30) { //reorder
                        ...
                        delay_buff->delay = 0;
                        fprintf(fp1, "time = %ld: [debug] Delayed Packet (%d bytes): ",
                            clock(), pkt_size);
                    } else { //drop
                        fprintf(fp1, "time = %ld: [debug] Dropped Packet (%d bytes): ",
                            clock(), pkt_size);
                        free(pkt);
                    }
                } else { //drop
        ...
        if (k == 0) {
            fprintf(fp1, "time = %ld: [debug] Sent Packet (%d bytes): ",
                clock(), tinytcp_pkt_size);
        } else {
            fprintf(fp1, "time = %ld: [debug] Sent Delayed Packet (%d bytes): ",
                    clock(), tinytcp_pkt_size);
        }
...
void* recv_from_network(void* args)
{
    ...
    while (1) {
        ...
        if (tail - head < LINK_DELAY_BUF_SIZE) {
            ...
            if (is_serv) {
                fprintf(stderr, "ALERT!!! link delay buffer overflow at server!! "
                        "Packet from client dropped!! "
                        "Possible reason: Your handle_recv_from_network() "
                        "implementation is too slow!\n");
            } else {
                fprintf(stderr, "ALERT!! link delay buffer overflow at client!! "
...
void* simulate_link_delay(void* args)
{
    ...
    while(1) {
        ...
        && tinytcp_hdr_size == TINYTCP_HDR_SIZE) {
            fprintf(fp2, "time = %ld: [debug] Recvd Packet (%d bytes): ",
                clock(), tinytcp_pkt_size);
...
int main(int argc, char* argv[])
{
    ...
    if (!strcmp(argv[1], "client")) {
        ...
        while (1) {
            ...
            if (num_of_closed_conn_client == tinytcp_conn_list_size) {
                break;
            }
        ...
        while (1) {
            ...
            for (int i = 0; i < tinytcp_conn_list_size; ++i) {
                ...
                if (num_of_closed_conn_server == tinytcp_conn_list_size) {
                    exit(1);
                }
